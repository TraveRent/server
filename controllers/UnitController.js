...
module.exports = class UnitController {
  static async postAddVendorUnit(req, res, next) {
    try {
      const {
        body: { name, brand, type, year, category, location, price },
        whoAmI: { _id },
        file: vehicle,
      } = req;
  ...
  static async putEditVendorUnitById(req, res, next) {
    try {
      ...
      res
        .status(200)
        .json({ message: "Successfully edit unit with id " + unitId });
    } catch (err) {
      next(err);
    }
  }
  ...
  static async deleteVendorUnitById(req, res, next) {
    ...
    res
      .status(200)
      .json({ message: "Successfully delete unit with id " + unitId });
